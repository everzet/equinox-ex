  ...
  Enum.each([Decider.Stateful, Decider.Stateless], fn decider_mod ->
    ...
    describe "#{inspect(decider_mod)}.transact/3" do
      ...
      test "executes decision callback and syncs the resulting events into a new state" do
        ...
        expect(StoreMock, :sync!, fn @stream, %{version: -1}, [2, 3], %{}, CodecMock, FoldMock ->
          State.new(5, 1)
        end)
      ...
      test "merges decider context with transact context and passes it all the way to store" do
        stub(FoldMock, :initial, fn -> 0 end)
        stub(StoreMock, :load!, fn @stream, %{version: -1}, _, _ -> State.new(0, -1) end)

        expect(StoreMock, :sync!, fn @stream, %{version: -1}, [2], ctx, CodecMock, FoldMock ->
          assert Map.has_key?(ctx, :allow_mocks_from)
          assert Map.has_key?(ctx, :custom_value)
          State.new(5, 1)
        end)

        decider = init(unquote(decider_mod), stream_name: @stream)
        assert {:ok, _} = Decider.transact(decider, fn 0 -> [2] end, %{custom_value: 2})
      end
      ...
      test "handles state<->stream version conflicts by reloading the state and redoing the decision" do
        ...
        expect(StoreMock, :sync!, fn @stream, %{version: 0}, [5], %{}, CodecMock, FoldMock ->
          State.new(7, 1)
        end)
